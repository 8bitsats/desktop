...
class ScrapybaraIntegration:
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        if not self.api_key:
            raise ValueError(
                "Scrapybara API key not found. "
                "Please provide it as an argument or set SCRAPY_API_KEY environment variable"
            )
    ...
    async def start_instance(self, instance_type: str = "ubuntu", timeout_hours: int = 1) -> Dict:
        ...
        Args:
            instance_type: Type of instance to start (ubuntu, browser, or windows)
            timeout_hours: Number of hours before instance auto-terminates

        Returns:
            Dict containing instance information
        """
        logger.info(f"Starting Scrapybara {instance_type} instance with {timeout_hours}h timeout")
        
        if instance_type.lower() == "ubuntu":
            self.instance = self.client.start_ubuntu(timeout_hours=timeout_hours)
        elif instance_type.lower() == "browser":
            self.instance = self.client.start_browser(timeout_hours=timeout_hours)
        elif instance_type.lower() == "windows":
    ...
    async def run_action(self, action_type: str, **kwargs) -> Dict:
        ...
        Args:
            action_type: Type of action (bash, computer, browser)
        ...
        if action_type == "bash":
            ...
            computer_action = kwargs.get("computer_action")
